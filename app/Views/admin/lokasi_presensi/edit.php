...
<div class="card col-md-6">
  <div class="card-body">
    <form method="POST" action="<?= base_url('admin/lokasi_presensi/update/'.$lokasi_presensi['id'])?>">
      ...
      <div class="input-style-1">
      <label>Radius</label>
      <input type="text" value="<?= $lokasi_presensi['radius']?>" class="form-control <?= ($validation->hasError('radius')) ? 'is-invalid' : '' ?>" name="radius" placeholder="radius" />
      ...
      <select name="zona_waktu" class="form-control <?= ($validation->hasError('zona_waktu')) ? 'is-invalid' : '' ?>">
        ...
        <option <?php if($lokasi_presensi['zona_waktu'] == 'WITA' ) {echo 'selected';}?> value="WITA">WITA</option><option <?php if($lokasi_presensi['zona_waktu'] == 'WIT' ) {echo 'selected';}?> value="WIT">WIT</option>
      </select>
